...
export const useWxJsSdk = (jsApiList = apiList, url = store.getters.initLink) => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const res = await wxJssdk({ url: decodeURIComponent(url) })
      wx.config({
        debug: false,
...
export const wxPay = (otpions: any) => {
  const { nonceStr, packageValue, paySign, signType, timeStamp } = otpions
  return new Promise((resolve, reject) => {
    wx.chooseWXPay({
...
export const wxScanQRCode = ({ needResult = undefined }) => {
  return new Promise((resolve, reject) => {
    wx.scanQRCode({
      ...
      success: function(res) {
        const result: string = res.resultStr // 当needResult 为 1 时，扫码返回的结果
        resolve(result)
      },
      fail: function(err) {
        reject(err)
